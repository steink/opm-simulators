                                    ...
                                    WellState<Scalar, IndexTraits>& well_state) const
{
    ...
    // don't do this for a converged well state
    if (ws.converged) {
        return;
    }
    ...
    // In this case, introduce small rates with fractions proportional to top rate to prevent 
    // problematic initialization of primary variables
    for (int seg = 0; seg < numberOfSegments(); ++seg) {
