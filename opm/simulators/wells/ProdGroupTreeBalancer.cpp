...
// ---------------------------------------------------------------------------
// Flatten an already-balanced tree for the network solve (groups_and_network_clean.md)
// ---------------------------------------------------------------------------
                       ...
                       FlatNetworkInput<Scalar>& out)
{
    ...
    // Wells directly under this Active node are weighed by their own guide
    // rate in *this* node's control mode -- the same lookup the balancer
    // itself uses (getGuideRateForMode), against the same beginning-of-step
    // initialRates snapshot, not the tree's own groupTarget.guideRate, which
    // is not reliably populated by the time balancing has converged.
    const auto ctrlMode = node.preferredMode;

    // Walk descendants: pure pass-through ones (Group/None/Transparent --
    // nothing of their own binds) keep flattening through; an Individual
    // node of either kind -- a well whose own limit binds (or, Part 1b, is
    // stopped) or a nested group with its own separate target -- stops the
    // flatten and becomes its own entry, referenced back as an active child
    // rather than folded into ownWells, since it has its own row (pinned, a
    // THP row, or its own group equation) that already determines it; this
    // node's sum still needs its total, just not tied to this node's lambda.
    std::function<void(const std::string&)> walk = [&](const std::string& childName) {
        if (tree.count(childName) == 0) {
            return;
        }
        const auto& child = tree.at(childName);
